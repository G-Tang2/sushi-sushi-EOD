...
export default function RollMoneyCounter({ denominations, rollsAtom }: RollsCounterProps) {
      ...
      {denominations.map(({ label, rollSize }) =>
        rollSize ? (
          <div key={label} className="flex items-center justify-center gap-1 w-36">
            <button onClick={() => decrement(label)} className="px-2 py-1 bg-gray-200 rounded" disabled={!rolls[label] || rolls[label] <= 0}>-</button>
            <input type="number" min="0" value={rolls[label] || ""} onChange={(e) => handleChange(label, Number(e.target.value))} className="no-spinner border p-1 w-16 text-center h-8 box-border leading-none" aria-label={`${label} rolls quantity`} />
